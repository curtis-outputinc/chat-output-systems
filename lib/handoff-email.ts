...
import { getSupabaseService, OUTPUT_TENANT_SLUG } from './supabase';
...
export async function sendHandoffEmail(payload: HandoffPayload): Promise<string | null> {
  ...
  const body = formatBody(payload, summary, insightsUrl);
...
function formatBody(
  ...
  if (payload.conversationId) {
    lines.push(
      `Full transcript: ${insightsUrl}/conversations/${payload.conversationId}`,
    );
  }
