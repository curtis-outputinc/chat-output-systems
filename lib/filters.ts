...
export function preFlightCheck(message: string): PreFlightResult {
  ...
  if (trimmed.length > MAX_USER_MESSAGE_LENGTH) {
    return {
      ...
      reason: 'too-long',
      publicMessage:
        "That's a long one, can you summarize the core question in a few sentences? Or book a call and we'll dig into the details there: https://cal.com/output-systems",
    };
  }
  ...
  for (const { pattern, label } of INJECTION_PATTERNS) {
    if (pattern.test(trimmed)) {
      return {
        ...
        publicMessage:
          "I'm here to answer questions about Output Systems and what we build. Anything I can help with on that?",
      };
    }
  }
...
/**
 * Output Systems stack vendor names. If the chatbot leaks any of these, that's
 * a violation per CLAUDE.md §6.4. We log the leak but don't redact (would
 * make responses incoherent); a logged leak prompts a corpus tightening.
 */
