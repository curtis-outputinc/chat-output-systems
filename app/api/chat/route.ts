...
export async function POST(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          } minutes, or book a call directly: ${CALCOM_URL}`,
    ...
    return NextResponse.json({ error: message, message: FALLBACK_MESSAGE }, { status: 500 });
  }
}
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      if (args.preFlight.blocked) {
        const publicMessage = args.preFlight.publicMessage ?? FALLBACK_MESSAGE;
      ...
      try {
        ...
        send({ type: 'replace', text: FALLBACK_MESSAGE });
      }
